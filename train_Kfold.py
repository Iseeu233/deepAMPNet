...
def parse_args():
    ...
    parser.add_argument('--dataset_h5', type=str, help='Path of the training dataset')
...
#    scheduler = optim.lr_scheduler.StepLR(optimizer, 2, 0.5)
    ...
    h5_dataset = StructureDataset(args.dataset_h5)
    labels = h5_dataset[:].y.numpy()
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(h5_dataset)), labels)):
        train_dataset = Subset(h5_dataset, train_idx)
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, drop_last=False)
        val_dataset = Subset(h5_dataset, val_idx)
        val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True, drop_last=False)

        outdir = os.path.join(args.save, str(fold + 1)+'fold')
        os.mkdir(outdir)
        save_name = os.path.join(outdir, str(fold + 1)+'fold')

        train(model, optimizer, loss_func, scheduler, train_loader, val_loader, save_name, args.epochs)
